...
const startDaemon = async () => {
  ...
  if (!config.rpc.secure) {
    ...
    const serverPems = generateCert('www.kws.com');
    ...
    const clientPems = generateCert('rpc.user.1');
    ...
    const subjectHash = '9099a931'; // openssl x509 -noout -subject_hash -in rpcuser1.cert.pem
  ...
  const cmd = path.join(global.resourcesPath, toExecutableName('bitcoinblack_node'));
  ...
  const pems = generateCert('rpc.user.1');
